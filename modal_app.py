...
def train_stage1(
    ...
    train_recs = [r for n in names for r in read_records(DATA_ROOT, n, "train")]
    val_recs = [r for n in names for r in read_records(DATA_ROOT, n, "val", val_limit)]
    print("train", len(train_recs), "val", len(val_recs), flush=True)
...
def train(
    ...
    history = train_stage1.remote(
        run_name, minutes=minutes, datasets=datasets, batch_size=batch_size, max_steps=max_steps,
        freeze_head=freeze_head, loss=loss, val_limit=val_limit, eval_every=eval_every, lr_proj=lr_proj, lr_head=lr_head,
    )
    for h in history:
        if "eval" in h:
            print(h["step"], h["eval"])
