//! Unicode range tables for the ByteLevel pre-tokenizer's regex.
//!
//! GENERATED by tools/gen_unicode_tables.py -- do not edit by hand.
//! Unicode 16.0.0, from Python 3.14.6 unicodedata.
//! Regenerate with: python3 ggml/tools/gen_unicode_tables.py ggml/src/unicode_tables.zig
//!
//! `letter` is \p{L} (677 ranges), `number` is \p{N} (144), `white_space` is the
//! White_Space property (10). Ranges are sorted, disjoint and inclusive.

/// Unicode version the tables below were generated from.
pub const version = "16.0.0";

/// True when `cp` lies in `rs`, a sorted, disjoint, inclusive range list.
pub fn inRanges(rs: []const [2]u32, cp: u32) bool {
    var lo: usize = 0;
    var hi: usize = rs.len;
    while (lo < hi) {
